...
class PostTMBlockGenerator : public Halide::Generator<PostTMBlockGenerator> {
   public:
    ...
    void generate() {
        ...
        output(x, y, ch) = cast<uint8_t>(select(
            alpha != 1,
            clamp((input_f(x, y, ch) - alpha * logo_f(x, y, ch)) / (1 - alpha),
                  0, 255),
            input_f(x, y, ch)));
    }
...
HALIDE_REGISTER_GENERATOR(PostTMBlockGenerator, tmblock_post)
